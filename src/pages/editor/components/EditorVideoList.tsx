...
import styles from "../../shared/components/VideoList.module.css"
...
export default function EditorVideoList({ title, videoList }) {
    const videos = videoList
        ? videoList.map((video) => getListItem(video))
        : null

    function getListItem(video) {
        return (
            <div
                className={[styles.videoInList, "listElement"].join(" ")}
                key={video.oid}
            >
                <div className={[styles.Title].join(" ")}>{video.title}</div>
                <img
                    className={[styles.Thumb].join(" ")}
                    src={video.thumb}
                ></img>
                <div className={[styles.buttonWrap].join(" ")}>
                    <Link
                        className={["button"].join(" ")}
                        to={`/editor/edit/${video.oid}`}
    ...
    return (
        <div className={[styles.videoListWrapper].join(" ")}>
            {title}
            {videos ? (
                <div className={[styles.videoListBody].join(" ")}>{videos}</div>
            ) : null}
        </div>
    )
}
