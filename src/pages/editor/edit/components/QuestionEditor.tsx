...
import questionStyles from "../../../app/videoPlayer/components/Question.module.css"
import styles from "./QuestionEditor.module.css"
import { MdAdd, MdClose } from "react-icons/md"
import TimeCodePicker from "./TimeCodePicker"
import TextareaAutosize from "react-textarea-autosize"
import { storeQuestion } from "../../../../utils/api/api"
import { getTimeInReadable } from "../../../../utils/utils"
...
export default function QuestionEditor({
    ...
    const singleChoiceRef = useRef(
        question.type == "singlechoice" ? true : false
    )
    ...
    function checkSingleChoiceAnswers() {
        if (question.type == "singlechoice") {
            ...
            if (correctCount == 1) {
                ...
                alert(
                    "Bei einer Single-Choice Frage muss genau eine Antwort richtig sein!"
                )
            }
        } else {
            return true
        ...
        return question.answers.map((answer) => (
            ...
            >
                {question.answers.length > 1 ? (
                    <div
                        className={[styles.circle, styles.delete].join(" ")}
                        onClick={() => deleteAnswer(answer.id)}
                    >
                        <MdClose></MdClose>
                    </div>
                ) : null}
                {question.correctAnswers.find((e) => e.id == answer.id)
                    .correct ? (
                    <div
                        className={[
                            styles.circle,
                    ...
                    <div
                        className={[styles.circle, styles.answerCorrect].join(
                            " "
                        )}
                        onClick={() => toggleCorrect(answer.id)}
                    >
                ...
                <TextareaAutosize
                    ...
                    value={answer.text}
                    onInput={(e) =>
                        handleAnswerChange(answer.id, e.currentTarget.value)
                    }
                ></TextareaAutosize>
            </div>
        ))
        ...
        <div className={["centerVertical", styles.wrap].join(" ")}>
            <div className={["center"].join(" ")}>
                <div className={["centerVertical", styles.timeWrap].join(" ")}>
                    <div>
                        Zeitpunkt der Frage:{" "}
                        <span className={[styles.timecode].join(" ")}>
                            {getTimeInReadable(question.timecode)}
                        </span>
                    </div>
                    <TimeCodePicker
                        duration={duration}
            ...
            <select
                ...
                onChange={(e) => {
                    ...
                    singleChoiceRef.current =
                        e.target.value == "singlechoice" ? true : false
                }}
            >
            ...
            <div
                className={[questionStyles.question, styles.stretch].join(" ")}
            >
                <div
                    className={[
                        questionStyles.questionElement,
                        questionStyles.questionText
                    ].join(" ")}
                >
                    <TextareaAutosize
                ...
                <MdAdd
                    className={[styles.addAnswer].join(" ")}
                    onClick={addAnswer}
                ></MdAdd>
